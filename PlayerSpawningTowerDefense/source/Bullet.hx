...
class Bullet extends FlxSprite 
{
	 ...
	 */
	public function init(X:Float, Y:Float, Target:Enemy, Damage:Int, Pierces:Bool):Void
	{
		reset(X, Y);
		startPos = new FlxPoint(X, Y);
		endPoint = new FlxPoint(256 * (Target.getPosition().x - X) + X, 256 * (Target.getPosition().y - Y) + Y);
		damage = Damage;
		pierces = Pierces;
	}
	...
	override public function update(elapsed:Float):Void
	{
		...
		FlxVelocity.moveTowardsPoint(this, endPoint, 200);
